...
* cin >> -- read value into the variable x from input stream
* cout << -- printf value to the output stream
...
```
* `#include` is a keyword which is used to include header files like : `<iostream>` and `<iostream.h>`. 
* `<iostream>` library file is used to read the data from terminal and to display the data on terminal. It has several in-built functions like cout, cin etc.
* `main()` function is the entry point of any C program.
* `cout and cin` are inbuilt library functions which are used for input and output in C++ language. They are defined in `iostream` header file.
...
|----|----|---|
| getline() | It allows you to enter multi-word string | `getline("string")`|
| puts() | It is used to show string output | `puts("string")`|
| length() | It is used to calculate the length of the string | `string_name.length()`|
| .compare() | It is used to compare two strings | `int k= str1.compare(str2)`|
| append() |It is used to concatenate two strings | ` str1.append(str)`|
| reverse() | It is used to return reverse of a string | `reverse("string")` |
| toupper() | It return string characters in uppercase | `toupper("string")` |
| tolower() | It return string characters in lowercase | `tolower("string")` |
...
These all the functions are available in `cstdlib`
...
|cmath | Mathematics functions|
|complex | Complex mathematics functions|
|climits | Size of basic types|
|ctime | C-style time/date utilites|
|limits | Uniform way to query properties of arithmetic types|
|string | strings are found in this header file|
|list | list container|
|map | multimap associative containers|
|queue | queue and std::priority_queue container adaptors|
